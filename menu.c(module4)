...
void execution (int *tab, int *n){
 int i = 0;
    ...
    char Fsnl[10000];
    FILE * Fpointer;
    Fpointer = fopen("Battements.csv","r"); //execution de fichier csv
  ...
  while( token != NULL ) {
      ...
      *n = *n+1; // La taille de tableau " tab "
      ...
      token = strtok(NULL, ";"); // continuer à enlever les ";" de notre chaine de caractere
      i++;
   }
   }
 }
void manipulation (int tab[],battm T[], int n ,int *n2 )
{
	...
	for (i=0;i<n;i=i+2)
{
   T[j].tmp=tab[i]; // Remplissage de laa partie de temps dans notre tableau de structure
 j++;
